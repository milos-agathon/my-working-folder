...
lapply(zipped_files, R.utils::gunzip)
...
terra::plot(country_lights_list[[1]])
...
terra::plot(country_lights_reproj[[1]])
...
terra::plot(country_lights_reproj2[[1]])
...
cols <- c("#1f4762", "#FFD966")
pal <- colorRampPalette(cols, bias = 3)(256)
...
for (i in 1:2) {
    file_name <- paste0("ukraine_", i, ".png")
    png(file_name)
    print(map[[i]])
    dev.off()
}
...
breaks <- classInt::classIntervals(
    country_lights_df[[1]]$value,
    n = 5,
    style = "equal"
)$brks

m1 <- ggplot(
    data = country_lights_df[[1]]
) +
    geom_tile(aes(
        x = x, y = y, fill = value
    )) +
